...
func main() {
	...
	err := godotenv.Load()
	if err != nil {
		// Пробуем альтернативные файлы конфигурации
		if errAlt := godotenv.Load("config.env"); errAlt != nil {
			log.Println("Предупреждение: не удалось загрузить файлы .env и config.env. Переменные окружения должны быть установлены иным способом.")
		} else {
			log.Println("Загружен альтернативный файл конфигурации: config.env")
		}
	}
